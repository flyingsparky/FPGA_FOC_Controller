...
//    .ch8			( 				),
    .busy			( adc_busy		),
    .fdata			( adc_fdata		),	// Unused for now
    .cvtData		( adc_cvtData	),
    .cs			    ( adc_cs_o		),
    .rd			    ( adc_rd_o		),
    .cvtA			( adc_cvtA_o	),
...
//reg signed [18:0] ia_temp, ib_temp, ic_temp;	// Because of scaling value may overflow when reading noisy inputs
reg signed [19:0] ia_temp1, ib_temp1, ic_temp1;
//localparam adc_bias = 16'sd10813;	// 1.65/5*2^15 （adc is +-5V range)
//localparam adc_bias = 16'sd11076;	// 1.69/5*2^15 due to bad 3v3 ldo smh
...
//		ic_temp <= 0;
		...
		if (adc_done) begin
			// ia_temp <= adc_b + adc_c - (adc_a <<< 1);
			// ib_temp <= adc_a + adc_c - (adc_b <<< 1);
			// ic_temp <= adc_a + adc_b - (adc_c <<< 1);
		...
		// ia_temp1 <= (ia_temp * adc_scale) >>> 13;
		// ib_temp1 <= (ib_temp * adc_scale) >>> 13;
		// ic_temp1 <= (ic_temp * adc_scale) >>> 13;
	end
end
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        // adc_fdata
        adc_fdata_sync  <= adc_fdata_i;
        adc_fdata       <= adc_fdata_sync;
        // adc_cvtData
