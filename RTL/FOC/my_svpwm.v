module my_svpwm (
	...
	input wire			[8:0]	v_amp,	// Must limit because of Deadtime wraparound + need time for adc
...
// Rectangular coordinates seems hard...
// wire [2:0] sector_arr [0:3];
// assign sector_arr = {3'd1, 3'd3, 3'd6, 3'd4};	// index {vy[15],vx[15]}
// sector <= ((vy[15]?-vy:vy) < ((SQRT3 * (vx[15]?-vx:vx)) >> 15)) ? sector_arr[{vy[15],vx[15]}] : (vy[15] ? 5 : 2);	//need to make sure vx/y inverted do not overflow
				// if (vy >= 0)
				// 	if (vx >= 0)
				// 		sector <= (vy < ((SQRT3 * vx) >> 15)) ? 1 : 2;
				// 	else
				// 		sector <= (vy < ((SQRT3 * -vx) >> 15)) ? 3 : 2;
				// else
				// 	if (vx >= 0)
				// 		sector <= (-vy < ((SQRT3 * vx) >> 15)) ? 6 : 5;
				// 	else
				// 		sector <= (-vy < ((SQRT3 * -vx) >> 15)) ? 4 : 5;